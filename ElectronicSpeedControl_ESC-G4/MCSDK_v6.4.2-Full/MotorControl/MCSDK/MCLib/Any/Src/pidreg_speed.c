...
/**
  ...
  *    * @f$\omega@f$: Electrical speed from delta angle (low pass filtered)
  *    * @f$\omega_{Emf}@f$: Electrical speed from angular speed minus correction signal (low pass filtered)
  * * Output:
...
fixp30_t PIDREG_SPEED2_run( PIDREG_SPEED_s* pHandle, const fixp30_t err, const fixp30_t errIncSpd)
{
  ...
  /* Proportional term saturated to 32bits to avoid overflow in PID output*/
  fixp24_t up = FIXP_mpyFIXPscaled_sat32(err_24, &pHandle->Kp_fps);

  /* Integral term */
  pHandle->dither = (pHandle->dither + 1) & 1;
  fixp24_t ui_increment = FIXP_mpyFIXPscaled_sat32(errIncSpd_24, &pHandle->Ki_fps);
  ui_increment = FIXP_add_sat32(ui_increment, pHandle->dither);
  ui = FIXP_add_sat32(ui, ui_increment);

  /* Saturation of the integral term to avoid wind-up in the PID */
  pHandle->clipped = ((ui <= min) || (ui >= max));
  ui = FIXP_sat(ui, max, min);
  pHandle->Ui = ui;

  fixp24_t sum = FIXP_add_sat32(up, ui);
  sum = FIXP_sat(sum, max, min);
  fixp30_t out = (sum << (PU_FMT-SUM_FTM));

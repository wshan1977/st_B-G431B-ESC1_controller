...
__weak bool STO_PLL_CalcAvrgMecSpeedUnit(STO_PLL_Handle_t *pHandle, int16_t *pMecSpeedUnit)
{
  ...
  else
  {
    ...
    int64_t lAux = (int64_t)wAvrSpeed_dpp * (int64_t)pHandle->_Super.hMeasurementFrequency; 
    lAux = lAux * ((int64_t)pHandle->_Super.SpeedUnit);
    lAux = lAux / ((int64_t)pHandle->_Super.bElToMecRatio);
    lAux = lAux / ((int64_t)pHandle->_Super.DPPConvFactor);
    if (lAux > INT16_MAX) 
    {
      wAux = INT16_MAX;
    }
    else if (lAux < INT16_MIN) 
    {
      wAux = INT16_MIN;    
    }
    else
...
/**
  ...
  * 
  * @param  pHandle: Handler of the current instance of the STO component.
  * @param  phForcedMecSpeedUnit: Mechanical speed in 0.1Hz unit as forced by VSS.

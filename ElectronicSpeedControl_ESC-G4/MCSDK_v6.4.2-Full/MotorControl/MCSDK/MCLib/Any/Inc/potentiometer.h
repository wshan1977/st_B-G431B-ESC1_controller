...
typedef struct
{
  ...
  uint8_t   LPFilterBandwidth;      /**< @brief Number of measures used to compute the average value
                                      ...
                                      * See #LPFilterBandwidthPOW2.
                                      */
  uint8_t   Index;                  /**< @brief position where the next measure will be put in #PotMeasArray */
 ...
 */
static inline uint16_t POT_GetValue(const Potentiometer_Handle_t *pHandle)
{
 ...
 * @param pHandle Handle on the Potentiometer component
 */
static inline bool POT_ValidValueAvailable(const Potentiometer_Handle_t *pHandle)
{
#ifdef NULL_PTR_CHECK_POT
    return ((NULL == pHandle) ? false : pHandle->Valid);
#else /* NULL_PTR_CHECK_POT */

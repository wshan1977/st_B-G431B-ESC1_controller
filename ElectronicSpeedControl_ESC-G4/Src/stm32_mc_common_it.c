...
void EXTI15_10_IRQHandler(void)
{
  ...
  if (0U != LL_EXTI_ReadFlag_0_31(LL_EXTI_LINE_10))
  {
    ...
    /* Debounce: a mechanical press fires several edges and each edge toggles
       start/stop, so one press could start then immediately stop the motor.
       Accept at most one edge per 300 ms. */
    {
      static uint32_t last_btn_tick = 0U;
      uint32_t now = HAL_GetTick();
      if ((now - last_btn_tick) > 300U)
      {
        last_btn_tick = now;
